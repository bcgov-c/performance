...
class UpdateUserManageAccessView4 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        ALTER VIEW user_manage_access_view
            ...
            SELECT 
                ...
                , (SELECT COUNT(DISTINCT ao.orgid) FROM admin_orgs AS ao WHERE ao.user_id = u.id AND ao.version = 2) AS org_count
            FROM ((users AS u USE INDEX (idx_users_employeeid_emplrecord), model_has_roles AS mhr, roles AS r) 
                LEFT OUTER JOIN employee_demo AS ed USE INDEX (idx_employee_demo_employeeid_name_email) ON ed.employee_id = u.employee_id AND ed.date_deleted IS NULL)
                LEFT OUTER JOIN employee_demo_tree AS odoh ON odoh.id = ed.orgid
