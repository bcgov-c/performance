...
class UpdateUserDemoJrView4 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        \DB::statement("
            ...
            FROM
                ...
                LEFT JOIN employee_demo_jr AS j ON (j.guid = u.guid AND j.id = (select max(j1.id) from employee_demo_jr AS j1 WHERE j1.guid = j.guid))
                ...
                LEFT JOIN user_reporting_tos AS rto ON 
                (
					u.id = rto.user_id
                    AND rto.reporting_to_id = (SELECT MIN(rto1.reporting_to_id) FROM user_reporting_tos AS rto1 WHERE rto1.user_id = rto.user_id)
				)
                LEFT JOIN users AS urt ON urt.id = rto.reporting_to_id
                LEFT JOIN employee_demo AS edo ON
                (
					urt.guid = edo.guid 
                    AND edo.employee_status = (SELECT MIN(edo1.employee_status) FROM employee_demo AS edo1 WHERE edo1.guid = edo.guid)
                    AND edo.empl_record = (SELECT MIN(edo2.empl_record) FROM employee_demo AS edo2 WHERE edo2.guid = edo.guid AND edo2.employee_status = edo.employee_status)
                )
            WHERE 
                NOT u.guid is null
                AND TRIM(u.guid) <> ''
