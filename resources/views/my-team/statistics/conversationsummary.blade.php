<x-side-layout title="{{ __('Statistic and Reports - Performance Development Platform') }}">
	...
	<form id="filter-form" class="no-print">
		...
		@include('my-team.statistics.partials.filter',['formaction' => route('my-team.statistics.conversationsummary') ])

	</form>

<h1 class="print-only">Goal Summary</h1>
<h5 class="print-only">Created on {{ \Carbon\Carbon::now()->format('M d, Y') }}</h5>
...
$(function() {
	...
	var allCharts = [];
	var export_url = '{{ route('my-team.statistics.conversationsummary.export') }}';  
