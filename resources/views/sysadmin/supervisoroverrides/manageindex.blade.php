<x-side-layout title="{{ __('Supervisor Overrides - Performance Development Platform') }}">
    ...
    <p class="px-3">Each row below is a separate supervisor override in the PDP. You can search by employee or supervisor to find the override you are looking for and view or delete as needed.</p>
        ...
        <script type="text/javascript">
			...
			$(document).ready( function() {
                ...
                $('#generictable').DataTable ( {
                    ...
                    stateSave: true,
                    ajax: {
                        url: "{{ route(request()->segment(1).'.supervisoroverrides.manageindexlist') }}",
                        type: 'GET',
                        data: function(d) {
                    ...
                    "fnRowCallback": function( row, data ) {
                        var index = $.inArray(data.override_id, g_selected_employees);
                        if ( index === -1 ) {
                            $(row).find('input[id="userCheck'+data.override_id+'"]').prop('checked', false);
                        } else {
                            $(row).find('input[id="userCheck'+data.override_id+'"]').prop('checked', true);
                        }
                    },
                    columns: [
                        {title: 'Override ID', ariaTitle: 'Override ID', target: 0, type: 'num', data: 'override_id', name: 'override_id', searchable: false, visible: false},
                        {title: ' ', ariaTitle: 'Override Checkboxes', target: 0, type: 'string', data: 'select_users', name: 'select_users', orderable: false, searchable: false},
                        ...
                        {title: 'Supervisor ID', ariaTitle: 'Supervisor ID', target: 0, type: 'string', data: 'supervisor_emplid', name: 'supervisor_emplid', searchable: true, className: 'dt-nowrap show-modal'},
                        {title: 'Supervisor Name', ariaTitle: 'Supervisor Name', target: 0, type: 'string', data: 'supervisor_name', name: 'supervisor_name', searchable: true, className: 'dt-nowrap show-modal'},
                        {title: 'Supervisor Email', ariaTitle: 'Supervisor Email', target: 0, type: 'string', data: 'supervisor_email', name: 'supervisor_email', searchable: true, className: 'dt-nowrap show-modal'},
                        ...
                        {title: 'Updated By', ariaTitle: 'Updated By', target: 0, type: 'string', data: 'updated_by_name', name: 'updated_by_name', searchable: false, className: 'dt-nowrap show-modal'},
                ...
                $('#delete-selected-btn').on('click', function() {
                    ...
                    if(confirm(message)) {
                        var deleteall_url = "{{ route(request()->segment(1) . '.supervisoroverrides.deletemultioverride', ':parray') }}";
