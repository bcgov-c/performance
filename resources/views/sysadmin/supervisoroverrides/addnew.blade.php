<x-side-layout title="{{ __('Supervisor Overrides - Performance Development Platform') }}">
    <div name="header" class="container-header p-n2 "> 
        <div class="container-fluid">
            <h3>Supervisor Overrides</h3>
            @include('sysadmin.supervisoroverrides.partials.tabs')
        </div>
    </div>	

    <p class="px-3">Administrators may override an employee's supervisor for a legitimate business reason. An employee can be delegated with only 1 supervisor override.</p>
        ...
        <div class="container-fluid">
            ...
            <p class="px-3">Use the search functions to find the employee(s). Select the employee(s) you want to manage and proceed to Step 2 below.</p>
            @include('sysadmin.supervisoroverrides.partials.loader')
            ...
            <div class="p-3">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-list" role="tabpanel" aria-labelledby="nav-list-tab">
                        @include('sysadmin.supervisoroverrides.partials.recipient-list')
                    </div>
        ...
        <form id="notify-form" action="{{ route(request()->segment(1).'.supervisoroverrides.saveall') }}" method="post">
        @csrf

        <div class="container-fluid">
            <br>
            <h6 class="text-bold">Step 2: Select a supervisor</h6> 
            <h6 class="px-3">Select a supervisor for the selected employee(s). Then proceed to Step 3.</h6> 
            ...
            @include('sysadmin.supervisoroverrides.partials.loader2')
            ...
            <div class="p-3">
                ...
                <div class="tab-content" id="enav-tabContent">
                    <div class="tab-pane fade show active" id="enav-list" role="tabpanel" aria-labelledby="enav-list-tab">
                        @include('sysadmin.supervisoroverrides.partials.erecipient-list')
                    </div>
        ...
        <div class="container-fluid">
            <br>
            <h6 class="text-bold">Step 3. Enter supervisor override reason</h6> 
            <br>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col col-12">
                            ...
                            <i class="fa fa-info-circle" label="Reason for sharing" data-trigger='click' data-toggle="popover" data-placement="right" data-html="true" data-content="Provide a brief explanation of why the employee(s) requires a supervisor override."> </i> 
                            <x-input id="input_reason" name="input_reason" :value="old('input_reason')"/>       
                                                 
                            @error('input_reason')
                                {{-- <div class="alert alert-danger alert-dismissable fade show"> "Reason for override" is required. </div> --}}
        ...
        <div class="container-fluid">
            <br>
            <h6 class="text-bold">Step 4. Save supervisor override for selected employees(s)</h6>
            <br>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-primary mt-2 sharebtn" type="button" name="btn_send" value="btn_send">Override</button>
    ...
    <x-slot name="js">
        ...
        <script>
            ...
            function confirmSaveAllModal(){
				$('#saveAllModal .modal-body p').html('Are you sure you want to override the supervisor for the selected employee(s)?');
				$('#saveAllModal').modal();
			}
            ...
            $(document).ready(function(){
                ...
                $('#employee-list-table').DataTable( {
                    ...
                    order: [[1, 'asc']],
                    ajax: {
                        url: "{{ '/' . request()->segment(1) . '/supervisoroverrides/employee-list/1' }}",
                        data: function (d) {
                ...
                $('#eemployee-list-table').DataTable( {
                    ...
                    order: [[1, 'asc']],
                    ajax: {
                        url: "{{ '/' . request()->segment(1) . '/supervisoroverrides/employee-list/2' }}",
                        data: function (d) {
				...
				$("#nav-tree-tab").on("click", function(e) {
                    ...
                    if(ddnotempty) {
                        ...
                        if($.trim($(target).attr('loaded'))=='') {
                            $.when( 
                                $.ajax({
                                    url: "{{ '/' . request()->segment(1) . '/supervisoroverrides/org-tree/1' }}",
                                    type: 'GET',
				...
				$("#enav-tree-tab").on("click", function(e) {
                    ...
                    if(ddnotempty) {
                        ...
                        if($.trim($(etarget).attr('loaded'))=='') {
                            $.when( 
                                $.ajax({
                                    url: "{{ '/' . request()->segment(1) . '/supervisoroverrides/org-tree/2' }}",
                                    type: 'GET',
                                    data: $("notify-form").serialize(),
                                    dataType: 'html',
                                    beforeSend: function() {
