...
class CreateAccessController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if($request->ajax()){
            ...
            $query = NotificationLog::when($date_sent_from, function ($query) use($date_sent_from, $date_sent_to) {
            ...
            return Datatables::of($query)
                ->addColumn('alert_type_name', function ($q) {
                    return $q->alert_type_name(); 
                })
                ->addColumn('alert_format_name', function ($q) {
                     return $q->alert_format_name(); 
                })
                ->addColumn('recipients', function ($q) {
                    $userIds = $q->recipients()->pluck('recipient_id')->toArray();
                    $users = User::whereIn('id', $userIds)->pluck('name');
                     return implode('; ', $users->toArray() );
                })
                ->addColumn('action', function ($q) {

                    return '<a href="#" class="notification-modal btn btn-xs btn-primary" value="'. $q->id .'"><i class="glyphicon glyphicon-envelope"></i>View</a>';
                })
        ...
        return view('sysadmin.access.createaccess', compact('criteriaList', 'roles', 'matched_emp_ids', 'old_selected_emp_ids') );
